...
bool Model::ray_box_intersect(const Vec3f &orig, const Vec3f &dir){
    float tmin = (min_box.x - orig.x) / dir.x;
    float tmax = (max_box.x - orig.x) / dir.x;

    if(tmin > tmax) std::swap(tmax, tmin);

    float tymin = (min_box.y - orig.y) / dir.y;
    float tymax = (max_box.y - orig.y) / dir.y;
    ...
    float tzmin = (min_box.z - orig.z) / dir.z;
    float tzmax = (max_box.z - orig.z) / dir.z;
